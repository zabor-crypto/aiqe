...
| `starter-example-no-false-green` | The published starter configuration cannot produce a pass when copied: every validator it declares names a command that does not exist, so they report `UNAVAILABLE` and their contracts `CONTRACT_UNKNOWN` | PROVEN | SUITE `test_documentation.py`, two tests: the file is parsed by the real fail-closed parser with all six families on a surface and bound to a required validator, and no validator may be a command that succeeds without checking anything | it is an illustration of shape, not a working configuration; the commands are the reader's to write | README, `config.md`, `examples/aiqe.toml` |
...
| `local-suite` | 961 passed, 0 failed, 0 errored, 21 skipped | OBSERVED | local run, this machine, 2026-09-07 | a local result; the surfaces CI proves are in MANIFEST | not published |
...
| `fold-platform-provenance` | The first fold states that macOS and Linux are proven by a retained full-lane release proof at the commit that manifest names, not the current one, and that this commit has had a Linux-only fast lane | PROVEN | MANIFEST `source_commit` and `python_support`, restating `python-range` and `ci-head-attestation` in the fold rather than adding a claim | a restatement, carrying provenance into the fold so a reader who stops there is not left with a claim about `HEAD`; it must never be shortened to "proven on macOS and Linux" | README (fold) |
