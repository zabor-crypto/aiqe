#!/usr/bin/env python3
"""Run the Task benchmark family and retain the result artifact.

    python3 bench/run-task-fixtures.py
    python3 bench/run-task-fixtures.py --output /tmp/fresh.json

Builds every case in `bench/fixtures/task/cases.json`, drives the real
...
`bench/results/task/results.json`.

A runner, not a framework: no plugins, no discovery, no reporters. `check`,
`receipt` and `commit` do not exist, so their families stay NOT_RUN and are
not fabricated here.

Exit status is 0 when every applicable case matched its expectation with all
three zero-tolerance quantities at zero and every negative control reproduced
its failure, and 1 otherwise.
"""
...
from fixtures.task import controls  # noqa: E402
from fixtures.task import harness  # noqa: E402

RESULTS_FILE = os.path.join(HERE, "results", "task", "results.json")

#: Fields recorded in the artifact but never compared between machines: they
#: are provenance, not claims, and they legitimately differ.
_NOT_RETAINED = ("start_head_sha", "started_at", "task_id")
...
def main(argv):
    ...
    for control in controls.CONTROLS:
        ...
        sys.stdout.write(
            "%-34s control_reproduces_failure=%s\n"
    ...
    sys.stdout.write(
        ...
        "repository mutations=%d state writes=%d executions=%d\n"
